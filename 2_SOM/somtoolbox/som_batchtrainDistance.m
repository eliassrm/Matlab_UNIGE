function [sMap,sTrain] = som_batchtrain(sMap, D, alpha, beta, varargin)
...
%   alpha   (scalar) number to use in superpixel
...
% Known = ~isnan(D);
% W1 = (mask*ones(1,dlen)) .* Known'; %W1 saved
% D(find(~Known)) = 0;
...
for t = 1:trainlen,
    ...
    % % constant matrices
    [maskside1 maskside2]=size(mask);
    mask1=mask(1, :);
    mask2=mask(2,:);
    mask3=mask(3,:);
    mask4=mask(4,:);
    i0 = 0;
    [msize1 msize2]=size(D);%training data i want to find dimensions
    D1=D(:, 1);%I take position in D1
    M1=M(:, 1);%I take position in M1
    D2=D(:, 2);%I take position in D2
    M2=M(:, 2);%I take position in M2
    D3=D(:,3);%I take x' in D3
    M3=M(:,3);%I take x' in M3
    D4=D(:,4);%I take x' in D4
    M4=M(:,4);%I take x' in M4
    WD1 = 2*diag(mask1)*D1';    % constant matrix
    WD2 = 2*diag(mask2)*D2';
    WD3 = 2*diag(mask3)*D3';
    WD4 = 2*diag(mask4)*D4';
    Known = ~isnan(D);
    [ksize1 ksize2]=size(Known);
    known1=Known(:, 1);%I take position in k
    known2=Known(:, 2);%I take position in k
    known3=Known(:,3);%I take x' in k
    known4=Known(:,4);%I take x' in k
    W11 = (mask1*ones(1,dlen)) .* known1'; %W11 saved
    W12 = (mask2*ones(1,dlen)) .* known2';
    W13 = (mask3*ones(1,dlen)) .* known3';
    W14 = (mask4*ones(1,dlen)) .* known4';
    D(find(~Known)) = 0;
    dconst = beta*((D1.^2)*mask1)'+beta*((D2.^2)*mask2)'+alpha*((D3.^2)*mask3)'+alpha*((D4.^2)*mask4)'; % constant in distance calculation for each data sample
    % W2 = ones(munits,1)*mask'; D2 = (D'.^2);
    % Dist = (M.^2)*mask*ones(1,d) + ones(m,1)*mask'*(D'.^2) - 2*M*diag(mask)*D'
    while i0+1<=dlen,
        inds = [(i0+1):min(dlen,i0+blen)]; i0 = i0+blen;
        %distance is M^2-2MD+D^2
        %distance is now beta(M1^2-2M1D1+D1^2+M2^2-2M2D2+D2^2) + alpha(M3^2-2M3D3+D3^2+M4^2-2M4D4+D4^2) alpha to
        %choose
        %Dist = (M.^2)*W1(:,inds) - M*WD(:,inds);%% distance super pixel
        Dist = beta*(M1.^2)*W11(:,inds) +beta*(M2.^2)*W12(:,inds) +(M3.^2)*W13(:,inds)*alpha+(M4.^2)*W14(:,inds)*alpha...
            - beta*M1*WD1(:,inds)- beta*M2*WD2(:,inds)-alpha*M3*WD3(:,inds)-alpha*M4*WD4(:,inds);
